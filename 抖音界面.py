...
class pcvideo_down(QThread):
    dive_list1=Signal(str,str,str,int,str)
    def run(self):
        self.headers = {
            "X-Argus": "",
            "User-Agent": "okhttp/3.10.0.1"
    ...
    def jiexi(self):
        ...
        if re.findall("暂时没有", req.text):
            ...
            for list in video:
                ...
                #data_move1[desc] = play_logo
                #print(stt)

                self.dive_list1[str, str, str, int, str].emit(name1, desc1, play_logo1, a1, stt1)
...
class ui:
    ...
    def insert_data1(self,name1,desc1,play1,a1,stt1):
        #print(name,desc)
        #self.video_data=data_move
    ...
    def cellClicked_TBW(self):
        ...
        if self.contents=="下载":
            ...
            if jiexi_f:
                ...
                # lujing=QFileDialog.getExistingDirectory(self)
                # print(down_url)
                self.thread4 = video_down()
                self.thread4.start()
                self.thread4.jindu.connect(self.jindu_show)
